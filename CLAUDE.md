...
- 🟡 Phase 5g.2 — PG HA with Patroni — Design ✅ 2026-05-24 (5g.2.a): decision rows landed (Patroni for HA mgmt, etcd co-located on HAProxy trio, no pgbouncer with revisit triggers, single HAProxy VIP for all consumers, Hlin/Eir/Snotra naming + sizing). Implementation sub-phases 5g.2.b–5g.2.j pending — HAProxy/etcd LXC trio (1133/1134/1135) → etcd cluster bootstrap → Vör/Idunn PG LXCs (1131/1132) → Patroni Fulla adoption + replica join → HAProxy + keepalived VIP `10.0.10.210` → Authentik cutover from IP-stopgap to VIP → failover validation.
...
- **Cloudflare Free plan: `Bot Management:Edit` is a separate API token permission, not folded under `Zone Settings:Edit`**: Even though Free's Bot Management surface is essentially just the `fight_mode` boolean, the `cloudflare_bot_management` Terraform resource (and the corresponding `/zones/{id}/bot_management` API endpoint) requires its own `Zone:Bot Management:Edit` scope. Adding `Zone Settings:Edit` alone is not sufficient — apply errors with auth failure on the bot_management resource until the dedicated scope is added. Per-zone scoping works (don't need account-wide). The `terraform-cloudflare` token currently has: `Zone:DNS:Edit + Zone:Zone:Read + Zone:Zone Settings:Edit + Zone:WAF:Edit + Zone:Bot Management:Edit` (all scoped to `xiiisins.com`) + `Account:Cloudflare Tunnel:Edit`. Discovered 2026-05-24 during Cloudflare re-harden.
- **Cloudflare provider v5: ruleset import ID requires the `zones/` discriminator prefix**: Provider v4 accepted `<zone_id>/<ruleset_id>` for `terraform import cloudflare_ruleset.foo`. Provider v5 demands `zones/<zone_id>/<ruleset_id>` (or `accounts/<account_id>/<ruleset_id>` for account-scoped rulesets). Error message is explicit when wrong: `invalid discriminator segment - <{accounts|zones}/{account_id|zone_id}>`. Same prefix-discriminator pattern applies to other v5 resources that span both zone and account scopes. Discovered 2026-05-24.
- **Authentik OIDC discovery lives per-app under `/application/o/<slug>/.well-known/openid-configuration`, NOT at host root**: Authentik supports multiple OIDC providers per instance, each with its own discovery document. There is NO host-root discovery doc — `https://authentik.example.com/.well-known/openid-configuration` is a hard 404, regardless of how many OIDC providers exist. The actual discovery URL is built from the issuer URL returned by WebFinger (or whatever directs the consumer): `<issuer>/.well-known/openid-configuration` → resolves under the per-app path. Implication for WAF/CDN skip rules, monitoring path lists, or anything that enumerates "the OIDC endpoints": use the `/application/o/*` prefix (covers per-app discovery + `/authorize/` + `/token/` + `/userinfo/` + per-app `/jwks/` + per-app `/end-session/` — all of Authentik's OIDC surface area). Discovered 2026-05-24 when an over-defensive skip rule pattern returned a confusing 503 (was actually pre-existing Authentik unhealth, but the dead pattern obscured the diagnosis).
