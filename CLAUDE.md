...
- Göndul moved Urd → Verd on 2026-05-17 (was deferred since the 2026-05-14 incident; finally fixed during the asgard rebuild). Future move back to Urd is Phase 4b, post-hardware-refresh.
...
- ✅ Urd hardware refresh — 2026-05-21. MSI Cubi replaces DeskMini JB95. i3-1215u + 1TB Lexar NM790 + reused 32GB DDR4. Original etcd-storm root cause (slow N5095 + mSATA fsync) removed. Phase 4b (Göndul Verd → Urd migration) deferred to a separate session.
- 🔲 Phase 4b — Göndul Verd → Urd migration. Deferred from Phase 4a session. Runbook in `docs/teardown-rebuild.md` Appendix B. Procedure: edit `terraform/proxmox/asgard-k3s/main.tf` `locals.control_planes.gondul.node = "urd"`, `terraform apply --target=...`, `kubectl delete node gondul`, run playbook with `-e 'k3s_init_node=hlokk'`.
...
- **Phase 4b deferred (Göndul Verd → Urd):** Future migration. Göndul stays on Verd until Phase 4b is run as a separate session. Procedure in `docs/teardown-rebuild.md` Appendix B.
...
- All IPs static
- Versions: intended to be pinned, with Renovate managing minor/patch via PR once the homelab reaches a working "2.0" state. The Ansible role versions ARE pinned (`k3s_version`, `calico_version`). HelmRelease charts were originally `version: "0.x"` placeholders — explicit pinning being a deliberate deferred step gated on the 2.0 state. **The 2026-05-21 Phase 4a deploy aged that position poorly:** two outages from floating pins in one session (metallb 0.16 unconditional `prometheus.serviceMonitor` reference; synology-csi 0.11.2's appVersion 1.3.0 image not published on Docker Hub). Both pinned to last-known-good (`metallb: 0.15.x`, `synology-csi: 0.11.1`). All other `0.x` chart pins should be tightened similarly, and the "wait for 2.0" position should be re-litigated — see homelab-design.md open questions.
...
- **Floating Helm chart `0.x` pins are a latent outage class**: The 2026-05-21 Phase 4a session ate two of these. (1) MetalLB chart `0.x` resolved to `0.16.0`, which adds an unconditional `.Values.prometheus.serviceMonitor.enabled` reference — no values block → template render fails. (2) synology-csi chart `0.x` resolved to `0.11.2`, which bumps appVersion to `v1.3.0` — and `synology/synology-csi:v1.3.0` is NOT published on Docker Hub (`NotFound`). Both were pinned to last-known-good (`metallb: 0.15.x`, `synology-csi: 0.11.1`). The "we'll pin everything at 2.0" deferral position is no longer credible — pin chart versions explicitly and let Renovate raise PRs.
