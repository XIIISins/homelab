...
- **IPAM/DCIM: NetBox in asgard K3s.** Source of truth for devices/VMs/LXCs/IPs/VLANs/prefixes (live since 5i, 2026-05-24). Git remains the IaC spec; NetBox is the queryable view. TF→NetBox standing pattern: every new LXC/VM TF resource MUST have matching `netbox_*` resources via `e-breuninger/netbox` provider (5i.3 to implement; current state retrofitted via `terraform import`). NetBox is K8s-fronted via Traefik at `netbox.niflheim.xiiisins.com` (internal-only). Authentik OIDC for login; permissions assigned manually in NetBox UI until SOCIAL_AUTH_PIPELINE override lands.
