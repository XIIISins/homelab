...
- **keepalived role logs `SECURITY VIOLATION - scripts are being executed but script_security not enabled`** on first start. Default behavior is scripts run as root (the keepalived process user). Pending role addition: `enable_script_security` global flag + `script_user keepalived_script` config + the user. Not a 5b.2 blocker; deferred. Captured 2026-05-25.
