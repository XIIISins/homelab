...
- 🔲 Jotunheim K3s
- 🔲 Services (Outline, Immich, n8n, Privatebin, Startpage)
...
- **`include_tasks` is dynamic — `--tags` doesn't propagate to inner tasks.** Only the include statement itself gets the tag at runtime; tasks inside the included file are loaded later and don't inherit. Symptom: `--tags postgres-common-databases` matches the include in `main.yml`, runs it, but `databases.yml`'s inner tasks all silently no-op (no error, PLAY RECAP shows `ok=1`). Contrast with `import_tasks` (static, tags propagate at parse time). **Rule for role authors**: NEVER use the bare `tags: [...]` form on an `include_tasks:` statement — it silently no-ops `--tags` selection and masquerades as working. ALWAYS use the `apply: tags: [...]` form inside the include (or convert to `import_tasks` if the included file doesn't need runtime-conditional inclusion):
  ```yaml
  - name: Render ...
    ...
    tags: [parent, sub-tag]
  ```
  Surfaced repeatedly: postgres-common split (commit 69d72d3), Teamspeak K3s deploy, Phase 7c.8 zbx_monitor (commit explicit fix), and the **pre-existing patroni-config include** caught + fixed during Phase 5h.2.j (commit c344697). `--tags patroni-config` had been silently no-op on every prior invocation. **Workaround for surgical re-runs of a role embedded in a multi-role play**: `--skip-tags <heavy-roles>` against the roles you don't want to re-execute, accept that all of the target role's tasks run.
...
- **`community.zabbix.zabbix_mediatype` schema has three load-bearing field-name surprises.** Wrong shape errors are visible only under `ANSIBLE_NO_LOG=False`; cost ~3 playbook iterations to surface during Phase 5h.2.i:
  - **`message_templates[].recovery` is plural** — `operations` / `recovery_operations` / `update_operations`. Singular `operation` is rejected. Zabbix API enum is plural; module passes through verbatim.
  - **`message_templates[].eventsource`, NOT `event_source`** — no underscore. Wrong shape triggers a `required together` error pointing at `eventsource, recovery`.
  - **For `type: webhook`, params field is `webhook_params`, NOT `parameters`.** Generic `parameters` is rejected with `Unsupported parameters for (community.zabbix.zabbix_mediatype) module: parameters` — the message helpfully lists everything supported (`webhook_params`, `event_menu` / `event_menu_name` / `event_menu_url` — note there is no `show_event_menu` boolean).
- **`community.zabbix.zabbix_user` is "create-OR-update" with creation-required fields, even on pure-update calls.** Adding a single entry to `user_medias` against an existing user (Admin) requires both `usrgrps` AND `passwd`. Without `usrgrps`: `state is present but all of the following are missing: usrgrps`. Without `passwd`: `User password is required. One or more groups are not LDAP based.` Workaround: re-state `usrgrps: ["Zabbix administrators"]` (no-op if already member) + provide `passwd` from Vault with `override_passwd: false` so it only applies on creation, never reset on subsequent reconciles. **Side effect**: if Admin's password is rotated via the Zabbix UI without updating Vault, the next role run won't force-revert (override_passwd=false), but Vault won't auto-learn either — rotate in Vault first, then UI. Surfaced 5h.2.i.
- **`zabbix_user.user_medias[].active` is BOOLEAN in the Ansible schema** (true/false), despite Zabbix's underlying API using `0=enabled, 1=disabled`. Passing the string `"enabled"` (matching the Zabbix UI label) errors with `argument 'active' is of type str ... we were unable to convert to bool`. The module translates true → "0" → enabled internally.
- **`zabbix_user.user_medias[].severity` defaults omitted keys to TRUE.** Six keys total (`not_classified`, `information`, `warning`, `average`, `high`, `disaster`); explicitly set each `false` you don't want firing. Setting only `{average: true, high: true, disaster: true}` produces bitmask 57 (= 56 + bit 0 for Not classified), not the desired 56. Cosmetic when paired with severity filtering elsewhere (e.g. JS suppression in the webhook script), but the explicit-false form reads cleaner in API state.
