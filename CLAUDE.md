...
- All three CPs (Göndul/Hlökk/Sigrún) bumped 1vCPU/2GB → **2vCPU/4GB** on 2026-05-17 during the Authentik deploy. Earlier doc text claimed Göndul was bumped during the asgard rebuild; the Terraform `locals.control_planes` map was unchanged until tonight. Authentik's first-deploy migration + blueprint-reconciliation burst on hlokk and sigrun confirmed the under-spec; symmetric 2vCPU/4GB across all three is now the standard (CP sizing identical across nodes, same rule as PG nodes — failover symmetry requires it).
- **CP-only workload posture (pending — Phase 4a, scheduled 2026-05-18 before Traefik+cert-manager):** CPs are not currently tainted, so workload pods can land on them. Tonight's deploy proved why this is wrong: workload pressure on a CP starved etcd of fsync I/O and dragged kubectl unresponsive cluster-wide. The fix is `node-role.kubernetes.io/control-plane=:NoSchedule` taint via the K3s config (`config-init.j2` + `config-server.j2`). After taint, CPs run only the control plane (etcd, apiserver, controller-manager, scheduler) plus DaemonSets that explicitly tolerate it (Calico-node, kube-proxy). The Synology CSI node-plugin should NOT tolerate it — desired outcome, fixes the cross-node iSCSI session-fight gotcha. 4 GiB is sufficient with this posture; the working set for control-plane-only is ~1.5-2 GiB, so 4 GiB gives proper headroom for bursts. Without the taint, 4 GiB was the bare minimum and bursts still hurt — taint is what makes 4 GiB actually be 4 GiB of headroom.
...
- **Node taints:** `node-role.kubernetes.io/control-plane=:NoSchedule` taint pending (Phase 4a, 2026-05-18). Set via `node-taint:` key in `config-init.j2` + `config-server.j2`. Until applied, workload pods can land on CPs — which empirically caused cascade failure during 2026-05-17 Authentik deploy. No labels set anywhere.
...
- Göndul: 2 vCPU, 4GB RAM, 10GB disk, on **Verd**
...
- **Pending:** `node-role.kubernetes.io/control-plane=:NoSchedule` taint (Phase 4a, 2026-05-18). Once applied, CPs run only control-plane components — workload pods cannot land on them under any condition. 4 GiB is sufficient with that posture. Without the taint, workload bursts on a CP starve etcd of fsync I/O — empirically confirmed during 2026-05-17 Authentik deploy.
...
- **iSCSI single-session LUNs**: Synology CSI LUNs are single-session. After ungraceful node restarts, a stale session or discovery node record can pin a LUN to a dead/wrong node and block re-attach (`non-retryable iSCSI login failure`, or `iscsi_limit_max_session_count` on the NAS side). Also: the Synology CSI node plugin is a DaemonSet and runs on CP nodes too (no CP taint yet) — a CP node grabbed a worker's LUN during a reschedule. Cleanup: `iscsiadm -m session` / `-m node -o delete` on affected nodes; clear stale sessions NAS-side if needed. **Workload concentration:** during the 2026-05-17 Authentik deploy, all 5 stateful workload PVCs (Vault ×3, Redis, one other) ended up colocated on `einherjar-skuld` due to LUN pinning across rebuilds. Load 1.84 vs 0.34 on peers. Not a hot bug but a structural pattern — StatefulSet pod spread doesn't override CSI's pinning once a LUN has been seen on a particular node. **Mitigation landing Phase 4a (2026-05-18):** CP taint `node-role.kubernetes.io/control-plane=:NoSchedule` evicts the Synology CSI node-plugin from CPs (it shouldn't tolerate the taint), eliminating the CP-grabs-worker-LUN failure mode. Pod anti-affinity for stateful workloads remains a separate consideration for the worker-concentration class.
