...
- 🔲 Phase 8c.3–8c.6 — SAML SSO + Traefik fronting + 3-path validation. Local-Admin via `hugin-direct.niflheim.xiiisins.com` is the only auth path until this lands.
...
- 🔲 Phase 6 — Human-facing secret management (Vault OIDC). Vault becomes the primary access point for *homelab-scoped* human-readable secrets, accessed via Vault UI with Authentik OIDC SSO at `vault.niflheim.xiiisins.com` (Traefik-fronted, niflheim Gateway). 1Password retains bootstrap creds + non-homelab + offline mirror discipline. Day-1 single-tier group mapping (Authentik `vault-admins` → Vault `homelab-admin` policy → `read+list secret/*`). Sub-phases 6.a (Authentik OIDC app + group) → 6.e (smoketest + discipline doc). Vault listener stays `tls_disable=1` (deferred). Side-effect: flipping `vault-ui` Service LoadBalancer→ClusterIP releases the dynamic-allocated MetalLB `.11` IP back to the pool.
