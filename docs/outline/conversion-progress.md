...
- **Pages:** 39 — the original 35 (Components & Interactions, Hardware, Services and purpose, Procedures, Troubleshooting + standalone URLs), the Startpage / MicroBin / n8n service subpages, and a Proxmox host patching procedure subpage — all added 2026-06-18 after merging `main` (96 commits) into the branch.
...
- **Published (2026-06-18):** `sync_outline.py` reconciled all pages into the live `Homelab` collection (token from Vault `secret/ansible/outline/api-token`, 1P mirror `[Asgard] - Mirror - Outline - admin API token`). All 39 pages live; sync is idempotent — a clean re-run is all-SKIP via the content-hash manifest `.outline-manifest.json`.
- **Cross-reference links:** now rewritten automatically at publish time (`rewrite_links` in the sync) — the bold canonical-name refs become real Outline `/doc/` links in nav sections + qualified inline refs, while emphasis bold is left alone. Source files keep the readable bold names. ~150 links across the collection.
- **Pending:** none on this branch. The two canonical-doc fixes (CLAUDE.md "iSCSI only" storage invariant; `notifications.md` uvicorn→gunicorn) are handled separately on `main`. Branch not pushed/merged back (operator's call).
