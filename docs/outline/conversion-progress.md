...
- **Pages:** 38 — the original 35 (Components & Interactions, Hardware, Services and purpose, Procedures, Troubleshooting + standalone URLs) plus Startpage / MicroBin / n8n service subpages, added 2026-06-18 after merging `main` (96 commits) into the branch.
...
- **Published (2026-06-18):** `sync_outline.py` reconciled all pages into the live `Homelab` collection (token from Vault `secret/ansible/outline/api-token`, 1P mirror `[Asgard] - Mirror - Outline - admin API token`). All 38 pages live; sync is idempotent — a clean re-run is all-SKIP via the content-hash manifest `.outline-manifest.json`.
- **Pending:** cross-reference link rewriting (the bold `**Name**` placeholders → real Outline `/doc/<id>` links — a manual reviewed pass; needs a curated name→doc alias map, deliberately not automated). Optionally a Procedures subpage for `proxmox-host-patching` (new on `main`, not yet converted — the Procedures set is a deliberately curated subset, so this is a judgement call). Branch is merged up to `main`; not pushed/merged back (operator's call).
