<!-- docs/procedures/archive/2026-05-17-asgard-rename-rebuild.md -->

# ⛔ ARCHIVED — executed runbook (historical record)

> **This is not the current procedure.** It documents the 2026-05-17 `must-run → asgard` rename+rebuild *as it was carried out*, scoped to the asgard K3s cluster only, against the homelab as it stood then (Vault on iSCSI, a single Vault KV entry, no PG/Patroni data tier, no Traefik/Authentik/observability/Garage/Outline/Semaphore/Zabbix, no storage-redesign tiering). It is kept verbatim as a validated reference for the K3s-cluster rebuild path and for the partial-rebuild appendices (single CP / single worker / Munin Tailscale), which remain accurate.
>
> **For the current disaster-recovery and full-rebuild procedure, see [`../teardown-rebuild.md`](../teardown-rebuild.md).** That guide supersedes this one: it covers the full homelab cold rebuild (foundation → LXCs → K3s → all services) in dependency order, with both Vault recovery paths and per-section time estimates.

---

# Disaster recovery & rebuild — asgard K3s (2026-05-17, executed)

> Originally the 2026-05-17 `must-run → asgard` rename+rebuild runbook. **Superseded as the canonical DR procedure** by [`../teardown-rebuild.md`](../teardown-rebuild.md). Retained as historical record + validated K3s-cluster-rebuild reference.
