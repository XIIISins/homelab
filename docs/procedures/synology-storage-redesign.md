...
- 🔲 Next: **Stage 3 — Volume1 shrink** (now unblocked: Volume1 has no live K8s LUN). DSM-delete the old meta PV/LUN (`pvc-fbc870f9`) when satisfied first; then relocate PBS datastore → Volume3 staging → delete+recreate Volume1 ~125 GB → restore PBS + verify → retire Volume3.
