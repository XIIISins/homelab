...
- ✅ **Class L Vault iSCSI → local-path DONE (2026-05-30)** — all 3 nodes migrated per-node, gated, leader last; cluster never lost quorum (≥2 voters throughout) and zero data loss. Method-of-record:
  - **Backup first:** `vault operator raft snapshot save` (full sealed-encrypted state, restorable) copied off-cluster to `~/homelab-backups/vault/` — *not* a plaintext `kv get` dump.
  - **Phase A (VCT swap, pods stay up):** edit HR `dataStorage.storageClass` → `local-path`; `flux suspend hr` → reconcile source+kustomization (HR object carries new values, helm held) → `kubectl delete sts vault --cascade=orphan` (3 pods keep running on iSCSI) → `flux resume hr` + `reconcile --force` → STS recreated on local-path VCT, re-adopts running pods. Existing PVCs untouched until per-node wipe.
  - **Phase B (per node, followers vault-2/vault-1 first, leader vault-0 last with `vault operator step-down`):** `raft remove-peer vault-N` (from a healthy node) → `delete pvc data-vault-N --wait=false` + `delete pod vault-N` → STS provisions fresh **empty** local-path PVC on the same worker (antiAffinity) → pod comes up uninitialized/not-ready → `kubectl exec vault-N -- vault operator raft join http://<leader>.vault-internal:8200` → KMS auto-unseals + resyncs.
  - **GATE between nodes:** wait for `raft autopilot state` **Failure Tolerance 1** AND the new node `Voter: true` — a freshly-joined node is a *non-voter* until autopilot promotes it (~stabilization window) and tolerance is **0** until then. Wiping the next node before promotion would lose quorum. (New gotcha → CLAUDE.md.)
  - KMS-on-empty-storage validated on vault-2 (the canary): uninitialized→joined→unsealed→standby in ~10s, no manual unseal needed.
  - iSCSI now **1 in use** (garage-meta only). 3 old vault PVs (`pvc-450130ba` v0, `pvc-e5b5e842` v1, `pvc-60a43ab8` v2) kept `Released`/`Retain` as rollback net → **DSM-delete when satisfied → cap 4/10 → 1/10**. Workers confirmed free of stale vault iSCSI sessions/node records.
- ✅ **DSM cleanup of Stage-1 + Class-L LUNs DONE (2026-05-30)** — operator DSM-deleted all 7 reclaimed LUNs (4 Stage-1 + 3 Vault); the 3 orphaned Released vault PV *objects* deleted in k8s too (the 4 Stage-1 PV objects were already gone). Cap **4/10 → 1/10** (garage-meta the only LUN). No released synology PVs remain.
- ✅ **Stage 2 — garage-meta → Volume2 (Class M) DONE (2026-05-30)** — the LMDB meta moved off the Volume1 default iSCSI SC onto Volume2's `synology-csi-iscsi-retain-vol2`, **kept block** (mmap must never hit NFS), via rsync static-rebind. Method-of-record:
  - Quiesce: `flux suspend kustomization apps,infrastructure` → `kubectl scale deploy outline -n outline --replicas 0` (dependent first) → `kubectl scale sts garage --replicas 0` (detaches meta+data).
  - Migrate: temp PVC `meta-vol2` on vol2 SC (10Gi, Immediate) → **privileged** migrator pod (`runAsUser:0`, reads source iSCSI PVC across MCS) `rsync -aHS --numeric-ids` (`-S` for LMDB sparse; no `-A/-X` per SELinux rule) → **checksum verify** (`sha256sum` all files minus `lost+found`, sorted-diff) = **MATCH** (9 files incl. `node_key`, `cluster_layout`, `db.lmdb/`).
  - Static-rebind (iSCSI Retain, identical to NFS path): delete migrator + temp PVC (vol2 PV → Released) → clear `claimRef` → delete old `meta-garage-0` PVC (old iSCSI PV → Released/Retain rollback net) → recreate `meta-garage-0` with `volumeName: <vol2PV>` → binds.
  - Edit STS `meta` VCT `storageClassName` → `synology-csi-iscsi-retain-vol2` (commit `7e9991a`) → reconcile source → `kubectl delete sts garage` (immutable VCT) → resume+reconcile infrastructure → STS recreated `replicas:1`, adopts rebound meta(vol2) + data(NFS). Resume apps + `scale outline --replicas 1`.
  - **Verified:** garage `/health` green (proves node has a role = layout intact), admin API GetClusterStatus node_id `d9d3d3e472a3…` unchanged + layout version 1 + bucket `outline` present with its access key still granted **read+write**; Outline `/_health` 200, no S3 errors. **Identity fully preserved** — same node, same layout, Outline reconnected seamlessly.
  - **iSCSI: Volume1 now holds ZERO live K8s LUNs** (only old meta PV `pvc-fbc870f9` Released/Retain as rollback net); Volume2 has the one bound LUN (`meta-garage-0` `pvc-84381dc4`). Storage-redesign LUN-vacate work is **complete** — every workload is on the tier its data needs.
- ✅ **Stage 3 — Volume1 delete+recreate DONE (2026-05-30)** — Volume1 reborn as the dedicated backup volume; PBS datastore moved back onto it + restore-verified. The redesign is **complete**. What it took (full retro: [`docs/incidents/2026-05-30-storage-redesign-nas-rework.md`](../incidents/2026-05-30-storage-redesign-nas-rework.md)):
  - PBS datastore moved Volume1 → **Volume3** (DSM folder move, ~74 GB) as staging; data verified intact on vol3 via a read-only scratch NFS mount (75 GB, full chunk store, all VM/CT groups incl. the worker VMs that hold the local-path Vault/VL/VM data).
  - Volume1 **wouldn't delete** (`Device or resource busy`) through a 5-holder hunt — iSCSI LUN → Tailscale package install dir → `s2s_daemon` (masked) → stale-PBS-mount `nfsd` lock → invisible `Synotify`/fanotify mark. **Resolved by a NAS reboot** (clears all kernel-level holds). Full diagnostic toolkit now in CLAUDE.md "Storage / iSCSI / Synology CSI".
  - **Pre-reboot quiesce** (NAS reboot bounces the one iSCSI target = garage-meta): `outline→0`, `garage→0` (clean iSCSI logout, no ext4-RO), suspend Flux apps+infra. garage came back **rw** + identity intact; teamspeak/garage-data NFS (`hard` mounts) self-recovered.
  - Volume1 recreated → PBS datastore moved back Volume3 → Volume1 → fstab was already `/volume1` (never changed), just `mount` + `systemctl restart proxmox-backup proxmox-backup-proxy` → **restore-verified** (operator: live LXC backup succeeded + file restore works; full restore-to-new-LXC deferred to next day).
  - **Tailscale outage** mid-Stage-3: reinstalling the Tailscale package (its dir was on old Volume1) + web-UI auth cut Munin's LAN → cut cluster storage → recovered via `tailscale down` over the tailnet IP → re-enabled with `--accept-routes=false --reset`. See CLAUDE.md "Tailscale".
  - **Cluster cleanup:** dangling PV `pvc-fbc870f9` deleted + stale iSCSI node-records for it cleared on urd+verd (skuld was already clean). Volume3 retire + dropping the cluster default StorageClass are the only Stage-4 tail items left.
