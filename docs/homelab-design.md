...
| Bifrost (Tailscale 1) | 1113 | Urd | `10.0.11.213` | Tailscale subnet router (advertises `10.0.0.0/16` supernet; indefinite key, `tag:subnet-router`). Primary of the bridge-pair. | 🔲 |
| Heimdall (Tailscale 2) | 1114 | Verd | `10.0.11.214` | Tailscale subnet router (same routes as Bifrost; indefinite key, `tag:subnet-router`). Guardian-of-the-bridge replica — naming-principle: primary defines theme, replica expands within it. | 🔲 |
| Gjallarbru (Tailscale 3) | 1115 | Skuld | `10.0.11.215` | Tailscale exit node (`--advertise-exit-node`; indefinite key, `tag:exit-node`). The bridge to Helheim — way out of the realm. | 🔲 |
...
- 🟡 **Phase 5e.3 — Tailscale OIDC blueprints + LXCs** (in progress 2026-05-23). Sub-phases 5e.3.a through 5e.3.c complete; remaining: Terraform tailscale ACL module, LXCs 1113/1114/1115, Munin DSM Tailscale package.
    ...
    **5e.3 sub-phase plan:**
    ...
    - [ ] **5e.3.e** — Tailscale LXCs (Bifrost 1113 / Heimdall 1114 / Gjallarbru 1115) in Proxmox via Terraform + Ansible. Bifrost+Heimdall are the subnet-router pair (Urd+Verd, `tag:subnet-router`, `--advertise-routes=10.0.0.0/16`); Gjallarbru is the exit node (Skuld, `tag:exit-node`, `--advertise-exit-node`). Debian 13 cloud-init, unprivileged, `/dev/net/tun` passthrough via new `enable_tun` variable on the existing LXC Terraform module. Broken into five sub-steps below — order is e.i first (module change is a prerequisite to e.iii's apply), then e.ii and e.iii can land in either order, then e.iv (needs both LXCs running and authkeys in Vault), then e.v.
        - [ ] **5e.3.e.i** — Extend `terraform/proxmox/asgard-lxcs/` LXC module with `enable_tun` boolean variable (default `false`). When `true`, adds a `device_passthrough` block for `/dev/net/tun` on the LXC resource. No resource churn on existing LXCs (Factorio + Fulla stay `enable_tun=false`).
        - [ ] **5e.3.e.ii** — In `terraform/tailscale/`, add three `tailscale_tailnet_key` resources (preauthorized, reusable=false, ephemeral=false, no expiry → indefinite) — one per LXC, each scoped to its target tag (`tag:subnet-router` for Bifrost+Heimdall, `tag:exit-node` for Gjallarbru). Write each authkey to Vault at `secret/k8s/tailscale/authkeys/<hostname>` per module-ownership rule. Authkeys are sensitive Terraform outputs; never logged to plan/apply summary.
        - [ ] **5e.3.e.iii** — In `terraform/proxmox/asgard-lxcs/lxcs.tf`, add a `tailscale_nodes` locals map + symmetric `proxmox_virtual_environment_container.tailscale` resource (mirrors the existing postgres pattern). Per-node placement: Bifrost on Urd, Heimdall on Verd, Gjallarbru on Skuld. Sizing: 1 vCPU / 512 MB / 4 GB disk (Tailscale's footprint is tiny). VLAN 11, `enable_tun=true`, `nesting=true` (systemd 257 gotcha). Apply.
