...
- [ ] **Phase 5e.3 — Tailscale OIDC blueprints + LXCs** (after 5e.2). Provider + application blueprints in Git (continuing the "blueprints from day 1, no UI config" rule). Tailscale ACL policy as code via `tailscale/tailscale` Terraform provider with `autoApprovers` for subnet-router/exit-node tags. LXCs 1113 (subnet router) + 1114 (subnet router, HA pair) + 1115 (exit node). Tailscale on NAS (Munin) for K3s-independent break-glass — app vs container TBD at deploy time. Unblocked by 5e.2 (publicly-reachable Authentik + WebFinger endpoint).
    ...
    **5e.3 prerequisites — pending decisions before the detailed plan can be drafted:**
    - [ ] **Tailscale account state.** Confirm tailnet exists / Free plan active / who's logged in / whether any existing devices are joined.
    - [ ] **Tailscale OIDC + WebFinger flow validation.** Before drafting LXC manifests: walk through the Tailscale tailnet-creation step against the existing WebFinger response and Authentik OIDC provider. If creation works, 5e.3 work is unblocked; if it surfaces a WebFinger or OIDC discovery shape problem, fix it BEFORE drafting LXCs (cheaper to fix the response than to roll back an LXC plan).
    - [ ] **LXC OS / template choice for Tailscale nodes.** Same Rocky Linux 9 cloud-init image used elsewhere, or something smaller (Debian / Alpine LXC)? Tailscale supports all three. Existing Ansible `baseline` role is RHEL-family — using a different OS means writing a parallel baseline.
    - [ ] **`/dev/net/tun` passthrough for unprivileged LXCs.** Tailscale needs TUN; unprivileged LXCs need explicit cgroup device rule + bind-mount. Existing Factorio + PostgreSQL LXCs are unprivileged but don't need TUN. Terraform LXC module needs a Tailscale-specific variant OR a conditional `features` block.
    - [ ] **Authentik OIDC provider/application blueprint shape.** Confirm scopes/claims Tailscale expects (`openid`, `profile`, `email`), redirect URI format, and whether to use a dedicated PropertyMapping for the `tailscale_groups` claim later.
    - [ ] **Munin Tailscale install path.** Container (Docker on DSM) or DSM app store package. Decision affects update story + auto-start integration.
