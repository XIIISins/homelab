...
- [ ] **Phase 5e.2 — Cloudflared + apex zone + WebFinger** (after 5e.1). Detailed plan to be drafted next session. Cloudflared in asgard K3s (tunnel creds via ESO from Vault), new `*.xiiisins.com` apex + `*.midgard.xiiisins.com` wildcards via cert-manager DNS-01 (same Cloudflare token, already correctly scoped — `Zone:DNS:Edit + Zone:Zone:Read` on `xiiisins.com`), second `midgard` Gateway in Traefik for the apex+midgard listeners (separated from `niflheim` by design — decision row above), WebFinger as static-response Traefik Middleware at `xiiisins.com/.well-known/webfinger`, Authentik HTTPRoute migrates off `niflheim` onto the new public Gateway + a midgard internal alias, AdGuard rewrite for `authentik.midgard.xiiisins.com` → `10.0.20.10`. Targets backend Services via ClusterIP DNS. Browser-visible cert externally is Cloudflare's universal cert (pattern (C)); our LE wildcards are origin-side.

    **5e.2 prerequisites — pending decisions before the detailed plan can be drafted:**
    - [ ] **Cloudflare account/zone confirmation.** Tunnel and DNS-01 cert token should share the same Cloudflare account; existing token is scoped to `xiiisins.com`. Worth confirming the tunnel will live in the same account/zone before drafting credential plumbing.
    - [ ] **Cloudflare Tunnel credential type.** Default to per-tunnel `credentials.json` (JWT, newer, what the `cloudflare/cloudflared` Helm chart expects) over legacy account-level `cert.pem`. Confirm or override.
    - [ ] **WebFinger / Authentik admin email.** `<mailbox>@xiiisins.com` — gets baked into the WebFinger `subject` field at tailnet creation AND must match the Authentik admin user identity. Email at `xiiisins.com` exists (Proton). Pick the local-part before drafting blueprint content.
    - [ ] **5e.2 service scope.** Authentik is locked in (required for Tailscale OIDC unblock). Decide whether other externally-exposed services (Outline, Immich, etc.) ride along in 5e.2 or each gets a follow-up phase. Default proposal: 5e.2 ships Authentik + WebFinger + plumbing only; other services land in their own phases once their internal-only deploys exist (most don't yet).
