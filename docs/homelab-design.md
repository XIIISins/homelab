...
- [ ] **Tighten `random` provider pin** in `terraform/proxmox/asgard-lxcs/versions.tf` (currently `~> 3.5`). Same IaC pin-policy debt as the `bpg/proxmox` closure above; missed in 5e.3.e.i. Trivial — set to concrete `3.7.2` (or whatever the lockfile resolves to) and `terraform init -upgrade=false` to confirm. Standalone commit.
