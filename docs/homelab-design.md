...
- 🟡 **Phase 5e.3 — Tailscale OIDC blueprints + LXCs** (in progress 2026-05-23). Sub-phases 5e.3.a through 5e.3.c complete; remaining: Terraform tailscale ACL module, LXCs 1113/1114/1115, Munin DSM Tailscale package.
    ...
    **5e.3 prerequisites — closed 2026-05-23:**
    ...
    - [ ] **`/dev/net/tun` passthrough for unprivileged LXCs.** Pending — Tailscale needs TUN; unprivileged LXCs need explicit cgroup device rule + bind-mount. Decision (revised 2026-05-23): hardcode a `device_passthrough { path = "/dev/net/tun" }` block directly on the new Tailscale LXC resource in `lxcs.tf` — original plan was a shared `enable_tun` variable but the code base has no LXC module wrapper (each resource is independent: Factorio is one-off, Postgres is locals+for_each), so a shared input variable wouldn't flow anywhere. bpg/proxmox PR #1722 (in v0.70.1+, well below our 0.106.0 pin) is the fix that makes device_passthrough work at create-time on non-clone containers. Lands in 5e.3.e.iii.
    ...
    **5e.3 sub-phase plan:**
    ...
    - [ ] **5e.3.e** — Tailscale LXCs (Bifrost 1113 / Heimdall 1114 / Gjallarbru 1115) in Proxmox via Terraform + Ansible. Bifrost+Heimdall are the subnet-router pair (Urd+Verd, `tag:subnet-router`, `--advertise-routes=10.0.0.0/16`); Gjallarbru is the exit node (Skuld, `tag:exit-node`, `--advertise-exit-node`). Debian 13 cloud-init, unprivileged, `/dev/net/tun` via hardcoded `device_passthrough` block on the new Tailscale resource (revised plan — see prereqs above). Broken into five sub-steps below — order is e.i first (pin-tighten gates everything else's apply), then e.ii and e.iii independent (either order), then e.iv (needs both LXCs running and authkeys in Vault), then e.v.
        - [ ] **5e.3.e.i** — Tighten `bpg/proxmox` provider pin in `terraform/proxmox/asgard-lxcs/versions.tf` AND `terraform/proxmox/asgard-k3s/versions.tf` from float `~> 0.77` to concrete `0.106.0` (the version currently resolved in the lockfiles — no upgrade, just locking in place). Closes the proxmox half of the IaC-pin-policy debt surfaced 2026-05-22. Doesn't require an apply, just `terraform init -upgrade=false` (or no-op `init`) on both roots to confirm the constraint is satisfied.
        - [ ] **5e.3.e.ii** — In `terraform/tailscale/`, add three `tailscale_tailnet_key` resources (`preauthorized = true`, `reusable = true`, `ephemeral = false`, `recreate_if_invalid = "always"`, `expiry = 7776000` — Tailscale API hard-caps at 90d, so "indefinite" is approximated via reusable + auto-recreate; see auth-key gotcha) — one per LXC, each scoped to its target tag (`tag:subnet-router` for Bifrost+Heimdall, `tag:exit-node` for Gjallarbru). Defensive `lifecycle { ignore_changes = [expiry] }` to avoid pre-#521 expiry-drift recreate-loops on provider 0.28.0. Description field constrained to alphanumeric + spaces + identifier-hyphens (`(`, `—`, `/` rejected by the API — see auth-key-description gotcha); use format `"<hostname> <tag> managed by terraform"`. Write each authkey to Vault at `secret/k8s/tailscale/authkeys/<hostname>` per module-ownership rule. Authkeys are sensitive Terraform outputs; never logged to plan/apply summary. New file `terraform/tailscale/authkeys.tf` (keeps `main.tf` focused on the ACL).
        - [ ] **5e.3.e.iii** — In `terraform/proxmox/asgard-lxcs/lxcs.tf`, add a `tailscale_nodes` locals map + symmetric `proxmox_virtual_environment_container.tailscale` resource (mirrors the existing postgres pattern). Per-node placement: Bifrost on Urd, Heimdall on Verd, Gjallarbru on Skuld. Sizing: 1 vCPU / 512 MB / 4 GB disk (Tailscale's footprint is tiny). VLAN 11, `features.nesting = true` (systemd 257 gotcha), and a hardcoded `device_passthrough { path = "/dev/net/tun" }` block inside the resource (NOT gated by a variable — see prereqs note). Apply.
        - [ ] **5e.3.e.iv** — New `ansible/roles/tailscale/` (Debian-only: add Tailscale apt repo + key, install `tailscale`, enable `tailscaled`, run `tailscale up --authkey=… --hostname=… --advertise-tags=… [--advertise-routes=… | --advertise-exit-node]` with per-host vars). Authkey pulled from Vault via `community.hashi_vault` lookup (same pattern as `secret/ansible/sftpgo/admin-password`). New `playbooks/tailscale-host.yml` (mirrors `factorio-host.yml`: baseline → tailscale → hardening). Inventory: add `tailscale_hosts` group under `asgard_lxcs` in `inventory/hosts.yml`, with `inventory/group_vars/tailscale_hosts.yml` carrying common config and per-host vars (`tailscale_tag`, `tailscale_advertise_routes`, `tailscale_advertise_exit_node`) inline on the host entries.
        - [ ] **5e.3.e.v** — Run playbook (bootstrap with `-e 'ansible_user=root' --tags baseline` then full apply as `ansible`). Validate: all three LXCs appear in the tailnet admin console; Bifrost+Heimdall's `10.0.0.0/16` route shows auto-approved; Gjallarbru's exit-node shows auto-approved; ping from a tailnet client (your laptop) to a homelab IP (e.g. `10.0.10.200` AdGuard VIP) succeeds through the subnet router; exit-node selection from the client routes egress through Gjallarbru.
