...
- Göndul moved Urd → Verd on 2026-05-17 (was deferred since the 2026-05-14 incident; finally fixed during the asgard rebuild). Phase 4b will return it to Urd post-hardware-refresh.
...
| 4b — Göndul Verd → Urd | 🔲 | Deferred from Phase 4a session. Returns Göndul to Urd post-hardware-refresh (the original 2026-05-14 placement, now valid again). Procedure in `docs/teardown-rebuild.md` Appendix B: edit `terraform/proxmox/asgard-k3s/main.tf` `locals.control_planes.gondul.node = "urd"`, `terraform apply --target=...`, `kubectl delete node gondul`, run playbook with `-e 'k3s_init_node=hlokk'`. |
...
| K3s on Urd | Worker only — historical reason removed 2026-05-21 | The original constraint (N5095 + mSATA too slow for etcd) is gone after the MSI Cubi swap. Phase 4b will move Göndul back to Urd. Until then, the "worker only" placement stays as the current cluster state, not as a hardware constraint. |
| Göndul placement | Verd 2026-05-17 → Urd planned for Phase 4b | Moved to Verd 2026-05-17 to escape Urd's etcd thrashing. Urd hardware refreshed 2026-05-21 (MSI Cubi); Phase 4b is the deliberate move back to Urd. CPs are 2vCPU/4GB symmetrically. |
...
| Helm chart pin policy | **Open — tighten now, not at 2.0** | Original position: `version: "0.x"` placeholders, real pinning gated on the homelab reaching a working "2.0" state. The 2026-05-21 Phase 4a session ate two outages from floating pins (metallb 0.16 broken template, synology-csi 1.3.0 image not published). Both pinned to last-known-good (`metallb: 0.15.x`, `synology-csi: 0.11.1`) during recovery. Remaining `0.x` pins are latent outages. Decision pending: tighten all chart pins to concrete or minor versions and enable Renovate now, rather than waiting for an arbitrary "2.0" milestone. |
...
- [ ] **Phase 4b — Göndul Verd → Urd migration.** Returns Göndul to Urd post-hardware-refresh. Deferred from Phase 4a session — deliberate, not opportunistic. Procedure: edit `terraform/proxmox/asgard-k3s/main.tf` `locals.control_planes.gondul.node = "urd"`, `terraform apply --target=module.proxmox_vms.proxmox_virtual_environment_vm.this["gondul"]`, `kubectl delete node gondul`, run playbook with `-e 'k3s_init_node=hlokk'`. Worth pre-flighting the helm-pin policy decision first (below).
- [ ] **Phase 5e.1 — Deploy Traefik + cert-manager** (after 4b). Let's Encrypt DNS-01 via Cloudflare API. Re-expose Authentik at `authentik.niflheim.xiiisins.com` over HTTPS. First real cert-manager exercise.
...
- [ ] **Helm chart pin policy — tighten now, not at 2.0?** Two outages in the 2026-05-21 session from `0.x` floating pins (metallb 0.16, synology-csi 1.3.0 image). Both pinned to last-known-good. Remaining `0.x` pins (sealed-secrets, vault, external-secrets, authentik) are latent outages. Decision: tighten all chart pins to concrete or minor versions now, enable Renovate now, accept PR churn — rather than waiting for the arbitrary "2.0" milestone. The "2.0 then pin" position assumed minor-version bumps wouldn't be breaking; both incidents disprove that.
...
- [x] **Urd hardware refresh** ✅ 2026-05-21. MSI Cubi (i3-1215u + 32GB DDR4 reused + 1TB Lexar NM790) replaces DeskMini JB95 (N5095 + 120GB mSATA). The original 2026-05-14 etcd-storm root cause is removed. Phase 4b will move Göndul back to Urd.

**K3s role correctness (from 2026-05-17 rebuild + 2026-05-21 Phase 4a):**
