...
- [ ] **Cached-NXDOMAIN flapping on internal FQDNs — root cause investigation.** Mitigated for Authentik 2026-05-24 by pointing `AUTHENTIK_POSTGRESQL__HOST` at Fulla's literal IP (`10.0.11.230`) — eliminates DNS from that connection path. Underlying class is still latent for any other K8s workload that resolves `*.niflheim.xiiisins.com` (or other internal zones) — pods landing during a brief upstream-DNS unavailable window will cache the NXDOMAIN and serve stale failures even after upstream recovers. Hypotheses to validate when investigation actually happens: CoreDNS NXDOMAIN cache TTL, pod resolver `options ndots`, AGH sync timing on Saga, K3s node `/etc/resolv.conf` fallback. Authentik's IP swap is a stopgap; the proper fix for this entire class lands when PG cluster expansion + HAProxy VIP allows Authentik to point at a stable VIP instead, and the same VIP pattern propagates to future services.
