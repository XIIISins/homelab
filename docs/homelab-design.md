...
- 🟡 **Phase 5e.3 — Tailscale OIDC blueprints + LXCs** (in progress 2026-05-23). Sub-phases 5e.3.a through 5e.3.c complete; remaining: Terraform tailscale ACL module, LXCs 1113/1114/1115, Munin DSM Tailscale package.
    ...
    **5e.3 sub-phase plan:**
    ...
    - [ ] **5e.3.e** — LXCs 1113 / 1114 / 1115 in Proxmox via Terraform. 3 LXCs: 1113 + 1114 subnet-router HA pair, 1115 exit-node. Debian 13 cloud-init template, unprivileged, `/dev/net/tun` passthrough via new `enable_tun` variable on the existing LXC Terraform module. Ansible role `roles/tailscale/` (new) for Tailscale daemon + authkey (minted in `terraform/tailscale/` via `tailscale_tailnet_key`, read from Vault) + tag application. Per-LXC config: 1113/1114 advertise `--advertise-routes=10.0.0.0/16 --advertise-tags=tag:subnet-router`; 1115 advertises `--advertise-exit-node --advertise-tags=tag:exit-node`.
