...
**OOB:** Tailscale DSM package (installed via Synology Package Center), tagged `tag:subnet-router`. Advertises the `10.0.0.0/16` supernet, auto-approved by the ACL's `autoApprovers.routes` rule — see Phase 5e.3.f.
...
| Tailscale plan + auth model | Free plan (3-user cap on custom OIDC). Split-auth: auto-renewing keys for servers (1113/1114/1115 + NAS) and one always-on user device — Tailscale's API caps key lifetime at 90 days, so "auto-renewing" means `reusable=true` + `recreate_if_invalid="always"` + max-allowed `expiry=7776000`, with the rebuild-an-LXC workflow being "TF apply this module first, then Ansible" (see auth-key gotcha). Default-expiry keys for phones/laptops via Authentik OIDC. | Decided 2026-05-22 (auto-renewing semantics clarified 2026-05-23 during 5e.3.e.ii planning when the 90-day cap was discovered). Free plan supports custom OIDC for up to 3 users (per Tailscale's own announcement) — enough for a personal homelab. Split-auth pattern: server-class devices never depend on Authentik for re-auth (they'd be locked out by an Authentik outage exactly when you need VPN access to fix it); user-class devices keep proper expiry rotation. The "one always-on device" lets the owner reach the tailnet without OIDC when traveling and Authentik is down. |
