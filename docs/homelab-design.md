...
**OOB:** Tailscale DSM package (installed via Synology Package Center), tagged `tag:subnet-router`. Advertises the `10.0.0.0/16` supernet, auto-approved by the ACL's `autoApprovers.routes` rule. Authkey is UI-minted and stored in 1Password (out-of-band — not in Vault, not auto-renewing); see decision row "Munin Tailscale authkey: out-of-band". DSM-7 TUN-permissions boot-task (`tailscale configure-host`) installed via DSM Task Scheduler — see CLAUDE.md gotcha for what wipes it. Deployed 2026-05-23 (Phase 5e.3.f); install procedure in `docs/teardown-rebuild.md` Appendix D.
...
| 5g.2 — PG HA with Patroni + etcd-on-HAProxy | 🟡 | Patroni-managed cluster (Fulla/Vör/Idunn), etcd co-located on HAProxy LXC trio (Hlin/Eir/Snotra at 1133/1134/1135), keepalived VIP `10.0.10.210` floating across the trio. No pgbouncer (documented decision with revisit triggers). Step-list tracked as checkboxes in pending tasks (per phase-structure rule). Once cutover lands, retires the Authentik IP stopgap; other planned consumers (Outline, Immich, NetBox, n8n, Zabbix, TeamSpeak) point at the same VIP when each lands. |
...
- [ ] **Cached-NXDOMAIN flapping on internal FQDNs — root cause investigation.** Mitigated for Authentik 2026-05-24 by pointing `AUTHENTIK_POSTGRESQL__HOST` at Fulla's literal IP (`10.0.11.230`) — eliminates DNS from that connection path. The Authentik-specific stopgap will be retired at Phase 5g.2.h when `AUTHENTIK_POSTGRESQL__HOST` cuts over from the literal IP to the HAProxy VIP `10.0.10.210` (still IP-form, no DNS in path — same NXDOMAIN-safe property). The underlying class is still latent for any other K8s workload that resolves `*.niflheim.xiiisins.com` (or other internal zones) — pods landing during a brief upstream-DNS unavailable window will cache the NXDOMAIN and serve stale failures even after upstream recovers. The VIP-not-FQDN pattern is the long-term structural answer (extend the "use VIP IPs, not internal-zone FQDNs, for K8s-to-LXC service traffic" rule); future PG consumers via the 5g.2 VIP get this for free. Hypotheses to validate if a non-PG case ever fires: CoreDNS NXDOMAIN cache TTL, pod resolver `options ndots`, AGH sync timing on Saga, K3s node `/etc/resolv.conf` fallback.
...
- [ ] **Split Tailscale resources out of `terraform/proxmox/asgard-lxcs/` into their own module.** Surfaced 2026-05-24 during 5g.2.b: the aliased `proxmox.root` provider (required for Tailscale's `device_passthrough` per Proxmox's ticket-auth-required-for-some-API-endpoints behavior) forces `PROXMOX_VE_PASSWORD` to be in env on *every* apply against this module, even when the apply only touches API-token-creatable resources (postgres, haproxy_etcd, factorio, future LXCs). The module is growing — splitting Tailscale to e.g. `terraform/proxmox/asgard-lxcs-privileged/` keeps the main module API-token-only and removes the recurring foot-gun. Migration: `terraform state mv` for the three Tailscale containers + their random_password resources, move the aliased provider block + the resource blocks, drop the `configuration_aliases` from versions.tf in the main module. Tailscale state surface is small (3 containers, stable — those LXCs aren't growing). Defer to a Terraform-structure-focused session.
...
- [ ] **Phase 5.5 — Doc tree restructure.** Scheduled between Phase 5 and Phase 6 (Phase 5 is critical infra, Phase 6 is optional Jotunheim — clean break). Migrate `docs/` from current flat layout (just `homelab-design.md` + `teardown-rebuild.md`) into a tree: `docs/procedures/` for ordered operational sequences (runbooks, deploy-a-VM, etc., composable across types), `docs/procedures/runbook/` for multi-day teardown-rebuild-class sequences (existing `teardown-rebuild.md` moves here), `docs/known-issues/` for gotchas (drawn from CLAUDE.md's section + incident log post-flight discoveries — duplication with CLAUDE.md acceptable, CLAUDE.md keeps gotchas for Claude's runtime context). Implementation plan in `homelab-design.md` rows become concise (one-line summaries + status tick) with operational detail living in `procedures/`. Code-adjacent `README.md` files (in role / module dirs) carry brief usage info — for an Ansible role this can be as small as "add to your roles list." Outline import handles the new tree natively (folders = sections). Phase-structure rule + content-separation rule both documented in CLAUDE.md "Phase structure & doc separation" subsection — this task is the migration to actually realize them on disk.
...
- 🟡 **PG cluster expansion → Phase 5g.2 — PG HA with Patroni.** Pulled forward from "after multiple consumers" to now, driven by operational pressure (Authentik PG-DNS flapping on 2026-05-23 mitigated by literal-IP stopgap; structural fix is the HA cluster + VIP). Architecture: Patroni-managed PG cluster (Fulla/Vör/Idunn) with etcd co-located on the HAProxy LXC trio (Hlin 1133/Urd, Eir 1134/Verd, Snotra 1135/Skuld), keepalived VIP at `10.0.10.210`, no pgbouncer (documented decision row with revisit triggers).
    ...
    **Step list** (checkbox form per phase-structure rule; commit hashes preserved as historical pointers — the original chat draft used a `5g.2.a/b/c/...` L4 naming that was retroactively recognized as a violation of the L3-max rule. Shipped commit subjects keep the L4 letters as historical record, doc tracking switches to checkboxes going forward):
    ...
    - [ ] Patroni role + Fulla in-place adoption + Vör/Idunn replica join (one role-write, one play handles both via Patroni's `bootstrap` config)
    - [ ] HAProxy + keepalived role + VIP `10.0.10.210` (Patroni REST API leader-detection)
    - [ ] Authentik cutover: `AUTHENTIK_POSTGRESQL__HOST` from literal `10.0.11.230` → VIP `10.0.10.210` (retires the IP stopgap from 2026-05-24)
    - [ ] Failover validation (kill leader, confirm HAProxy reroutes + Patroni elects new leader + Authentik survives)
    - [ ] Post-flight docs + commit bundle (build-sequence tick, decision-log entries if any new ones surfaced, gotchas from the implementation work)
