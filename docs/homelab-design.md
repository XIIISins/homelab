...
| WebFinger hosting | Traefik static-response Middleware on the `midgard` Gateway at `xiiisins.com/.well-known/webfinger` | Decided 2026-05-22 during 5e.2 design. WebFinger is required for Tailscale custom OIDC tailnet creation (`{ "subject": "acct:<email>", "links": [{ "rel": "openid issuer", "href": "https://authentik.xiiisins.com/application/o/tailscale/" }] }`). Required only at tailnet creation per Tailscale docs ("Tailscale will remember the OIDC config"), but kept always-on for future re-creation flows. Alternatives considered: tiny static-content pod (ConfigMap-backed), Cloudflare Pages (state outside cluster). Middleware option keeps everything in the same IaC stack — no extra pod, no out-of-cluster state. |
...
| Tailscale ACL as code | Terraform `tailscale/tailscale` provider, autoApprovers on `tag:subnet-router` + `tag:exit-node` | Decided 2026-05-22. ACL policy file (JSON) source-of-truth in Git, reconciled by Tofu Controller via the same Flux-driven path as `terraform/vault/`. Avoids manual admin-console clicks for subnet route approval — devices joining with the right tag self-approve their routes. Fallback procedure for the breakglass case (NAS joins with a fresh authkey before ACL has reconciled): manual approval in admin console. Procedural step documented in the runbook. Alternatives considered: JSON file in Git + manual sync via CLI (rejected — drift risk, no obvious reconcile signal). |
...
- [ ] **Phase 5e.2 — Cloudflared + apex zone + WebFinger** (after 5e.1). Cloudflared in asgard K3s (locally-managed tunnel — `config.yaml` ingress rules in Git as ConfigMap, per-tunnel `credentials.json` via ESO from Vault), new `*.xiiisins.com` apex + `*.midgard.xiiisins.com` wildcards via cert-manager DNS-01 (same Cloudflare DNS-01 token, already correctly scoped — `Zone:DNS:Edit + Zone:Zone:Read` on `xiiisins.com`), second `midgard` Gateway in Traefik for the apex+midgard listeners (separated from `niflheim` by design — decision row above), WebFinger as static-response Traefik Middleware at `xiiisins.com/.well-known/webfinger`, Authentik HTTPRoute migrates off `niflheim` onto the new public Gateway + a midgard internal alias, AdGuard rewrite for `authentik.midgard.xiiisins.com` → `10.0.20.10`. Targets backend Services via ClusterIP DNS. Browser-visible cert externally is Cloudflare's universal cert (pattern (C)); our LE wildcards are origin-side.
    ...
    **5e.2 sub-phase plan:**
    - **5e.2.a** — Bootstrap `terraform/cloudflare/` module. Cloudflare provider, new Terraform-purpose API token (scope: `Zone:DNS:Edit + Zone:Zone:Read` on `xiiisins.com` + `Account:Cloudflare Tunnel:Edit`), token stored in 1Password + local env (NEVER in Terraform state). New module, separate from cert-manager's existing zone-only token.
    - **5e.2.b** — Two new Certificates in `cert-manager-config/`: `*.midgard.xiiisins.com` then `*.xiiisins.com` apex. Staging → prod for each (same 5e.1 pattern). Reuses existing `letsencrypt-staging` + `letsencrypt-prod` ClusterIssuers.
    - **5e.2.c** — Second `midgard` Gateway in `traefik` namespace, lives in `gateway-config/` alongside the existing `niflheim` Gateway. Apex + midgard listeners, references the two new cert Secrets, `allowedRoutes.namespaces.from: All`.
    - **5e.2.d** — Terraform: create tunnel object + DNS record `authentik.xiiisins.com CNAME <uuid>.cfargotunnel.com`. Push `credentials.json` into Vault at `secret/k8s/cloudflared/credentials`. Tunnel UUID becomes config input for step e.
    - **5e.2.e** — Cloudflared K8s manifests in `infrastructure/cloudflared/`: namespace, ExternalSecret pulling `credentials.json` from Vault, ConfigMap (`config.yaml` — ingress: `authentik.xiiisins.com → https://traefik.traefik.svc.cluster.local:443` with `originRequest.httpHostHeader: authentik.xiiisins.com` + `noTLSVerify: true`), Deployment (3 replicas, hostname anti-affinity, no autoscaling — Cloudflare guidance: scale-down breaks existing connections). Targets Traefik (not Authentik direct) so WebFinger Middleware applies.
    - **5e.2.f** — WebFinger Traefik Middleware: static-response, JSON body with `acct:ghost@xiiisins.com` subject + Authentik OIDC issuer pointer, attached on midgard Gateway's apex listener at path `/.well-known/webfinger`. Required for Tailscale custom-OIDC tailnet creation; once-only at creation per Tailscale docs but kept always-on for future re-creation.
    - **5e.2.g** — New Authentik HTTPRoute attaching to midgard Gateway with hostnames `authentik.xiiisins.com` + `authentik.midgard.xiiisins.com`. **Keep old niflheim HTTPRoute live** during this step. Update Authentik brand blueprint to match new hostnames (broaden domain field or add new entries). Cutover safety: Authentik reachable on both old and new during validation.
    - **5e.2.h** — AdGuard rewrite: `authentik.midgard.xiiisins.com → 10.0.20.10`. Apex `authentik.xiiisins.com` deliberately NOT rewritten — internal clients use midgard FQDN, external clients let public DNS resolve to the tunnel.
    - **5e.2.i** — End-to-end validation: internal via `https://authentik.midgard.xiiisins.com`, external via `https://authentik.xiiisins.com` (from mobile data or off-LAN network), WebFinger via `curl https://xiiisins.com/.well-known/webfinger?resource=acct:ghost@xiiisins.com`. Confirm all three paths before cleanup.
    - **5e.2.j** — Cleanup: delete old `authentik.niflheim.xiiisins.com` HTTPRoute + matching AGH rewrite. Restore HelmRelease `install.remediation.retries: 3` on cloudflared if first-deployed with `-1` (per 5e.1 pattern).
- [ ] **Phase 5e.3 — Tailscale OIDC blueprints + LXCs** (after 5e.2). Provider + application blueprints in Git (continuing the "blueprints from day 1, no UI config" rule). Tailscale ACL policy as code via `tailscale/tailscale` Terraform provider with `autoApprovers` for subnet-router/exit-node tags. LXCs 1113 (subnet router) + 1114 (subnet router, HA pair) + 1115 (exit node). Tailscale on NAS (Munin) for K3s-independent break-glass — app vs container TBD at deploy time. Unblocked by 5e.2 (publicly-reachable Authentik + WebFinger endpoint).
...
- [ ] **Restore HelmRelease `install.remediation.retries: 3`** on cert-manager + Traefik (currently `-1` for first-deploy debugging). One commit covers both.
...
- [ ] Decide which services get external Cloudflare Tunnel exposure (in addition to direct port-forwards via UCG). Standing decision; pick services as 5e.2 deploys. First entry locked in: Authentik (for Tailscale OIDC).
