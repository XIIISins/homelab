...
locals {
  vms = {
    ...
    # ── Hermod — notifications (LXC 1103, Phase 5h.2) ──────────────
    hermod = { vmid = "1103", role = "monitoring", device = "verd", cpu = 1, memory = 512, primary_iface = "eth0" }
  ...
  vm_interfaces = {
    ...
    # PBS + Hugin (Zabbix) + Hermod (notifications) + AGH trio + Tailscale trio + Factorio + PG trio (single-homed VLAN 11)
