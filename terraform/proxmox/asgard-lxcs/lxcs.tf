...
# ----------------------------------------------------------------------------
# LXC 1103 — Hermod (notifications, Verd)
# ----------------------------------------------------------------------------
# Norse identity: Hermod, Odin's messenger — rode to Hel to plead for Baldr's
# return. The homelab's notification messenger: sources POST one JSON shape,
# Hermod fans out to Discord (and future delivery channels) by tag.
#
# Stack:
#   - caronc/apprise-api via Docker — single Python/uvicorn process
#   - /etc/apprise/config/homelab.yml — tag-driven URL routing
#     (critical/alert/media → distinct Discord webhooks)
#   - Stateless on the LXC root disk (PBS-backed) — no DB, no PVC
#   - Internal-only on VLAN 11 — no Tailscale, no Cloudflare
#
# Placement: Verd, NOT Urd or Skuld. Independent failure domain from K3s
# (asgard cluster owns Urd's CP) AND from PG leader bias (Skuld carries
# Fulla pre-failover). Verd is the "neutral middle" for a service whose
# whole job is to fire when other things break.
#
# Sizing: 1 vCPU / 512 MB / 4 GB. AppriseAPI is single-digit RSS at
# steady state. Bumps later if needed.
#
# See:
#   - docs/services/notifications.md (full design — tag taxonomy,
#     JSON schema, source-side mapping table)
...
resource "proxmox_virtual_environment_container" "hermod" {
  description = "Hermod — notification fan-out (AppriseAPI)"
  ...
  memory {
    dedicated = 512 # MB — AppriseAPI is stateless Python, single-digit RSS
    swap      = 512
  }

  disk {
    datastore_id = var.lxc_storage
    size         = 4 # GB — apprise-api Docker image + config + logs
  }
  ...
  features {
    nesting = true # systemd 257 on Debian 13 + Docker-in-LXC needs nesting
  }
