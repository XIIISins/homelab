...
# -----------------------------------------------------------------------------
# Hermod Discord webhooks (Phase 5h.2)
# -----------------------------------------------------------------------------
# Three placeholder paths the operator overwrites with real Discord webhook
# values (minted in the Discord UI for #infra-critical / #infra-alerts /
# #media channels). Each path holds two fields: `id` (the webhook ID) +
# `token` (the webhook token). lifecycle.ignore_changes prevents future TF
# applies from clobbering the operator-written values back to placeholders.
#
# Discord webhook URL format:
#   https://discord.com/api/webhooks/<id>/<token>
# Operator extracts the id + token segments and:
...
#
# Tag-driven fanout config in roles/hermod-api/templates/homelab.yml.j2
# reads these via Vault lookup at deploy time and renders the Apprise
# config (one URL per tag).

resource "vault_kv_secret_v2" "hermod_discord_critical" {
  mount = vault_mount.kv.path
  name  = "ansible/hermod/discord/critical"
  data_json = jsonencode({
    id    = "placeholder-pending-discord-webhook"
    token = "placeholder-pending-discord-webhook"
  })
  lifecycle {
    ignore_changes = [data_json]
  }
...
resource "vault_kv_secret_v2" "hermod_discord_alert" {
  mount = vault_mount.kv.path
  name  = "ansible/hermod/discord/alert"
  data_json = jsonencode({
    id    = "placeholder-pending-discord-webhook"
    token = "placeholder-pending-discord-webhook"
  })
...
resource "vault_kv_secret_v2" "hermod_discord_media" {
  mount = vault_mount.kv.path
  name  = "ansible/hermod/discord/media"
  data_json = jsonencode({
    id    = "placeholder-pending-discord-webhook"
    token = "placeholder-pending-discord-webhook"
  })
